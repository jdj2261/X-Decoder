...
class XDecoder(nn.Module):
    ...
    def forward(
        ...
        if task == "captioning_infer":
            return self.forward_captioning(
                x,
                mask_features,
                mask=mask,
        ...
        if self.training and task == "vlp" and self.task_switch["captioning"]:
            ...
            caping_lang_embed = torch.cat(
                [caption["caption_tokens"] for caption in target_vlp], dim=0
            ).transpose(
                0, 1
            )  # language output
            _caping_lang_embed = caping_lang_embed.detach().clone()
            output = torch.cat(
                (output, _caping_lang_embed), dim=0
            )  # concat object query, class token and caption token.
            caping_lang_embed += self.pos_embed_caping.weight.unsqueeze(1).repeat(
                1, bs, 1
            )
            query_embed = torch.cat(
                (query_embed, caping_lang_embed), dim=0
            )  # may not add at the beginning.
            self_tgt_mask = self.self_attn_mask.repeat(
                output.shape[1] * self.num_heads, 1, 1
            )
        elif (
            (self.training and task == "seg") or (task == "grounding_eval")
        ) and self.task_switch["grounding"]:
            self_tgt_mask = self.self_attn_mask[
                :, : self.num_queries, : self.num_queries
            ].repeat(output.shape[1] * self.num_heads, 1, 1)
            ...
            pad_tgt_mask = (
                torch.ones(
                    (
                        ...
                        self.num_queries
                        + (self.num_queries - 1)
                        + len(grounding_tokens),
                        self.num_queries
                        + (self.num_queries - 1)
                        + len(grounding_tokens),
                    ),
                    device=self_tgt_mask.device,
                )
                .bool()
                .repeat(output.shape[1] * self.num_heads, 1, 1)
            )
            ...
            pad_tgt_mask[
                :, self.num_queries :, self.num_queries :
            ] = False  # grounding tokens could attend with eatch other
            self_tgt_mask = pad_tgt_mask
            output = torch.cat((output, output[:-1]), dim=0)
            query_embed = torch.cat(
                (query_embed, query_embed[:-1]), dim=0
    ...
    def forward_captioning(
        ...
        for cap_idx in range(0, self.captioning_step):
            ...
            output = torch.cat(
                (query_feat, caping_lang_embed), dim=0
            )  # concat object query, class token and caption token.
            caping_lang_embed += pos_embed_caping
            query_embed = torch.cat(
                (query_embed_, caping_lang_embed), dim=0
            )  # may not add at the beginning.
        ...
        out = {"pred_captionings": caping_lang_token, "pred_texts": texts_new}
        return out
    ...
    def forward_prediction_heads(
        ...
        if self.task_switch["captioning"] and (
            task == "vlp" or task == "captioning_infer"
        ):
            outputs_captionting = (
                decoder_output[:, self.num_queries :] @ self.caping_embed
            )
        else:
        ...
        sim = (cls_token @ obj_token.transpose(1, 2)).softmax(-1)[
            :, 0, :, None
        ]  # TODO include class token.
        ...
        if (
            ...
            decoder_output = torch.cat(
                (
                    decoder_output[:, : self.num_queries - 1],
                    cls_token,
    ...
    @torch.jit.unused
    def _set_aux_loss(
        self, outputs_class, outputs_seg_masks, outputs_boxes, outputs_captions
    ):
