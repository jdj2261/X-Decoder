...
name2folder = {
    "scannet_41_val_seg": "label41",
    "scannet_38_val_seg": "label38",
    "scannet_21_val_seg": "label21",
}

name2class = {
    "scannet_41_val_seg": SCAN_40,
...
def load_scannet_instances(
    ...
    for field in fileids:
        ...
        r = {
            ...
            "image_id": semseg_dir.split("/")[-3]
            + semseg_dir.split("/")[-1].split(".")[0],
        }
        dicts.append(r)
    return dicts
...
def register_scannet_context(name, dirname, split, class_names=name2class):
    DatasetCatalog.register(
        name, lambda: load_scannet_instances(name, dirname, split, class_names)
    )
