...
class OfficialVCOCOEvaluator(DatasetEvaluator):
    """
    ...
    def __init__(
        ...
        self.VCOCO = self._load_vcoco(vsrl_annot_file)
    ...
    def process(self, inputs, outputs):
         ...
         for img_results, img_targets in zip(outputs, targets):
            for hoi in img_results['hoi_prediction']:
                ...
                if img_results['predictions'][hoi['object_id']]['category_id'] == self.missing_category_id:
                ...
                for idx, score in zip(hoi['category_id'], hoi['score']):
                    verb_class = self.verb_classes[idx]
                    score = score.item()
    ...
    def evaluate(self, ovr_thresh=0.5, eval_types=['scenario_1']):
        map = {}
    ...
    def _do_role_eval(self, vcocodb, dets, ovr_thresh=0.5, eval_type='scenario_1'):
            ...
            for aid in range(self.num_actions):
                ...
                for rid in range(len(self.roles[aid])-1):
                    ...
                    for j in idx:
                        ...
                        overlaps = self.get_overlap(gt_boxes, pred_box)
                        ...
                        if np.all(gt_roles[jmax, :] == -1): # if no gt role
                            ...
                            ov_role = self.get_overlap(gt_roles[jmax, :].reshape((1, 4)), role_boxes[j, :])
        ...
        for aid in range(self.num_actions):
            ...
            for rid in range(len(self.roles[aid])-1):
                ...
                role_ap[aid, rid] = self.voc_ap(rec, prec)
        ...
        role_ap[-3][0] = np.nan # remove point_inst
        print('Average Role [%s] AP = %.2f, omitting the action "point"'%(eval_type, np.nanmean(role_ap) * 100.00))
        print('---------------------------------------------')
