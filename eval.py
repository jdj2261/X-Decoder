...
def main(args=None):
    ...
    # build model
    model = (
        BaseModel(opt, build_model(opt)).from_pretrained(opt["WEIGHT"]).eval().cuda()
    )
        ...
        if is_main_process():
            for eval_type in results.keys():
                for key in results[eval_type]:
                    scores["{}/{}/{}".format(dataset_name, eval_type, key)] = results[
                        eval_type
                    ][key]
                    if key in summary_keys:
                        summary[
                            "{}/{}/{}".format(dataset_name, eval_type, key)
                        ] = results[eval_type][key]
